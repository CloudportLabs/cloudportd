 ...
 *| |    | |/ _ \| | | |/ _` | '_ \ / _ \| '__| __|
 *| |____| | (_) | |_| | (_| | |_) | (_) | |  | |_ 
 * \_____|_|\___/ \__,_|\__,_| .__/ \___/|_|   \__|
 *                           | |                   
 *                           |_|                   
 *
 * 
 *  Cloudport Daemon v1 Rigged
 *  (c) 2025 is.a.duck or Duck1405 and contributers
 * 
*/

/**
 * @fileoverview Main entry file for the Cloudport Daemon. This module sets up an
            ...
            if (file.endsWith('.js')) {
                try {
                    ...
                    if (typeof router === 'function' && router.name === 'router') {
                        ...
                        app.use(`/`, router);
                        log.info(`Loaded Module: ${routeName}`);
                    } else {
                        log.warn(`${file} isn't a Module. Not loading it`);
                    }
                } catch (error) {
                    log.error(`Error loading Module from ${file}: ${error.message}`);
                }
            }
        });
    });
    ...
    wss.on('connection', (ws, req) => {
        ...
        function authenticateWebSocket(ws, req, password, callback) {
            if (password === config.key) {
                ...
                ws.send(`\r\n\u001b[33m[cloudportd] \x1b[0mconnected!\r\n`);
                const urlParts = req.url.split('/');
                const containerId = urlParts[2];
        ...
        async function performPowerAction(ws, container, action) {
            ...
            if (!actionMap[action]) {
                ws.send(`\r\n\u001b[33m[cloudportd] \x1b[0Invalid action: ${action}\r\n`);
                return;
            }
        
            ws.send(`\r\n\u001b[33m[cloudportd] \x1b[0mWorking on ${action}...\r\n`);
        
            try {
                await actionMap[action]();
            } catch (err) {
                console.error(`Error performing ${action} action:`, err);
                ws.send(`\r\n\u001b[33m[cloudportd] \x1b[0Action failed: ${err.message}\r\n`);
            }
        }
...
app.get('/', async (req, res) => {
    try {
        ...
        const response = {
            versionFamily: 1,
            versionRelease: 'cloudportd ' + config.version,
            online: true,
        ...
        res.status(500).json({ error: 'Docker is not running - cloudportd will not function properly.' });
    }
});

app.use((err, req, res, next) => {
    console.error(err.stack);
    res.status(500).send('Something has... gone wrong!! You are Fucked!!');
});
...
setTimeout(function (){
  server.listen(port, () => {
    log.info('cloudportd is listening on port ' + port);
  });
}, 2000);