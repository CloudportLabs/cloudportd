...
router.post('/fs/:id/files/edit/:filename', async (req, res) => {
    ...
    try {
        ...
        if (!isEditable(filePath)) {
            return res.status(400).json({ message: 'File type not supported for editing, try uploading something else!!' });
        }
        await fs.writeFile(filePath, content);
        res.json({ message: 'Your file has been updated successfully!!' });
    } catch (err) {
