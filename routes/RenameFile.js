...
router.post('/fs/:id/files/rename/:filename/:newfilename', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            return res.status(400).json({ message: 'A file with the new name already exists, make a new one!!' });
        } catch (err) {
        ...
        if (err.code === 'ENOENT') {
            res.status(404).json({ message: 'File not found!! Refresh your browser!!' });
        } else {
